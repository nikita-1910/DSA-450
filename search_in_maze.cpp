...
void findPathHelper(int m[][MAX], int n, int x, int y, int dx[], int dy[], string path){
    if (x == n - 1 && y == n - 1) {
        res.push_back(path);
        return;
    }
...
vector<string> findPath(int m[][MAX], int n){
    // dx, dy will be used to follow `DLRU` exploring approach which is lexicographically sorted order
    int dx[] = { 1, 0, 0, -1 };
    int dy[] = { 0, -1, 1, 0 };
    if (m[0][0] == 1) {
        m[0][0] = 2;
